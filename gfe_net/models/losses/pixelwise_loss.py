...
class SpatialFrequencyLoss(nn.Module):
    def __init__(self, loss_weight,reduction,lambda_fre=0.1):
    ...
    def forward(self, pred, gt):
        # ---- Spatial domain loss ----
        l_spa = self.l1(pred, gt)

        # ---- Frequency domain loss ----
        # Compute FFT for both prediction and ground truth
        ...
        # Amplitude and phase
        pred_amp = torch.sqrt(pred_real ** 2 + pred_imag ** 2 + 1e-8)
        gt_amp = torch.sqrt(gt_real ** 2 + gt_imag ** 2 + 1e-8)
        pred_phase = torch.atan2(pred_imag, pred_real)
        gt_phase = torch.atan2(gt_imag, gt_real)

        # L1 differences
        l_fre_amp = self.l1(pred_amp, gt_amp)
        l_fre_phase = self.l1(pred_phase, gt_phase)

        l_fre = l_fre_amp + l_fre_phase
