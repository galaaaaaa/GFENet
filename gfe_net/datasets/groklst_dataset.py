...
class GrokLSTDataset(BaseDataset):
    """GrokLSTDataset for open source projects in OpenMMLab/MMagic.
    ...
    METAINFO = dict(dataset_type="GrokLSTDataset", task_name="Downscaling")
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if self.use_ann_file:
            for idx in path_list:  # idx
                ...
                if self.hr_guidance is not None:
                    hr_guidance_path = osp.join(
                        self.data_prefix["hr_guidance"], hr_guidance
                    )
