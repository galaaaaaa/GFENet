        ...
        for filename in filenames:
            if isinstance(self.data_field_name, str):
                try:
                    ...
                    data = mat_data[self.data_field_name]
                except KeyError:
                    print("\n" + "="*50)
                    print(f"[DEBUG 捕获异常] 也就是这里报错了！")
                    print(f"1. 正在尝试读取的文件: {filename}")
                    print(f"2. 代码想要找的变量名: '{self.data_field_name}'")
                    print(f"3. 文件里实际存在的变量名: {list(mat_data.keys())}")
                    print("="*50 + "\n")
                    raise
